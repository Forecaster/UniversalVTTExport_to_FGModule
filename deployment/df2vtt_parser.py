...
def main(module_name, files, refine_portals=False, door_width=10, grid_color="00000000", author="DungeonFog", extension="mod", cleanup=True, extract=False):
	...
	for file in files:
		...
		for portal in data["portals"]:
			...
			if portal_type != "passage":
				...
				if portal_type != "toggleable_wall" and portal_type != "illusory_wall":
					expand_points = expand_line(pos_x(portal["bounds"][0]["x"]), pos_y(portal["bounds"][0]["y"]), pos_x(portal["bounds"][1]["x"]), pos_y(portal["bounds"][1]["y"]), door_width)
		...
		images.append(generate_image(image_path, filename, ".png", pixels_per_grid, grid_color, occluders, offset_x, offset_y, lights))
		cleanup.append(image_path)
		counter += 1
